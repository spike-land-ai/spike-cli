...
import type { AuthTokens } from "../../src/auth/token-store.js";
...
describe("token-store", () => {
  ...
  describe("isTokenExpired", () => {
    it("returns false when no expiresAt", () => {
      expect(isTokenExpired({ ...sampleTokens, expiresAt: undefined })).toBe(
        false,
      );
    });
    ...
    it("returns false when token is still valid", () => {
      ...
      expect(isTokenExpired({ ...sampleTokens, expiresAt: future })).toBe(
        false,
      );
    });
