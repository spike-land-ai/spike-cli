...
} from "../../src/chat/slash-commands.js";
import type { SlashCommandContext } from "../../src/chat/slash-commands.js";
import type {
  NamespacedTool,
  ServerManager,
} from "../../src/multiplexer/server-manager.js";
import type { ChatClient, Message } from "../../src/chat/client.js";
import { AppRegistryImpl } from "../../src/chat/app-registry.js";
import type { AppInfo } from "../../src/chat/app-registry.js";

// Helper to create a mock tool
function mockTool(
  overrides: Partial<NamespacedTool> & { namespacedName: string; },
): NamespacedTool {
  return {
...
function createMockManager(tools: NamespacedTool[] = []): ServerManager {
  return {
    ...
    getServerTools: vi.fn().mockImplementation((name: string) =>
      tools.filter(t => t.serverName === name).map(t => ({
...
describe("parseSlashInput", () => {
  ...
  it("parses a tool invocation with JSON args", () => {
    const result = parseSlashInput(
      "/chess_create_game {\"time_control\": \"RAPID_10\"}",
    );
...
describe("extractPrefix", () => {
  ...
  it("uses custom separator", () => {
    expect(extractPrefix("spike---chess_create_game", "spike", "---")).toBe(
      "chess",
    );
  });
});
...
describe("auto-fill from session state", () => {
  ...
  it("does not override user-provided args", async () => {
    ...
    await handleSlashCommand(
      "/chess_make_move {\"game_id\":\"user_game\",\"from\":\"e2\",\"to\":\"e4\"}",
      {
        manager,
        client,
        messages: [],
...
describe("handleSlashCommand direct tool invocation", () => {
  ...
  it("returns error for invalid JSON args", async () => {
    const tools = [
      mockTool({ namespacedName: "s__some_tool", serverName: "s" }),
    ];
    const manager = createMockManager(tools);
    const client = createMockClient();
