...
import { type AgentLoopContext, runAgentLoop } from "../../src/chat/loop.js";
import type {
  ChatClient,
  ContentBlock,
  Message,
...
describe("runAgentLoop", () => {
  ...
  it("handles tool_use → tool_result → text response", async () => {
    ...
    const stream1 = createMockStream([
      toolUseBlock("t1", "vitest__run_tests", { filter: "*.ts" }),
    ]);
    const stream2 = createMockStream([textBlock("All tests passed!")]);

    mockClient = {
      createStream: vi.fn().mockReturnValueOnce(stream1).mockReturnValueOnce(
        stream2,
      ),
      model: "claude-sonnet-4-6",
    } as unknown as ChatClient;
