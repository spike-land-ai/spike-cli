...
import type { AliasConfig } from "../../src/alias/types.js";
...
describe("AliasResolver", () => {
  ...
  describe("resolveTool", () => {
    it("resolves a tool alias", () => {
      const resolver = new AliasResolver(
        makeConfig({ tools: { rt: "vitest__run_tests" } }),
      );
    ...
    it("real tool name beats tool alias", () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        expect.stringContaining("shadows a real tool"),
      );
  ...
  describe("resolveServer", () => {
    it("resolves a server alias", () => {
      const resolver = new AliasResolver(
        makeConfig({ servers: { prod: "production" } }),
      );
