...
import type { ServerManager } from "../../src/multiplexer/server-manager.js";
...
async function withHttpServer(
  ...
  const { startHttpServer } = await import("../../src/transport/http-server.js");
...
async function withSseServer(
  ...
  const { startSseServer } = await import("../../src/transport/sse-server.js");
...
describe("HTTP server — API key validation", () => {
  ...
  it("allows requests with the correct api key", async () => {
    ...
    const res = await fetch(`${baseUrl}/mcp`, {
      ...
      body: JSON.stringify({ jsonrpc: "2.0", method: "initialize", id: 1, params: { protocolVersion: "2024-11-05", capabilities: {}, clientInfo: { name: "test", version: "1" } } }),
    });
    // Should not be 401
...
describe("HTTP server — createMcpServer", () => {
  it("creates a server with ListTools handler that returns tools from manager", async () => {
    ...
    const { createMcpServer } = await import("../../src/transport/http-server.js");
