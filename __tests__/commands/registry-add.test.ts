...
import { registerRegistryCommand } from "../../src/commands/registry.js";
...
describe("registry add command", () => {
  ...
  it("exits with 1 when server is not found in registry", async () => {
    ...
    const program = makeProgram();
    await program.parseAsync(["registry", "add", "unknown-server"], { from: "user" }).catch(() => {});

    expect(errorSpy).toHaveBeenCalledWith(
      expect.stringContaining('"unknown-server" not found'),
    );
    expect(exitSpy).toHaveBeenCalledWith(1);
  });
  ...
  it("exits with 1 when server has no url or command", async () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      expect.stringContaining("no url or command"),
    );
    expect(exitSpy).toHaveBeenCalledWith(1);
  });
