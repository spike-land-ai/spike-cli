...
} from "../../src/completions/installer.js";
...
describe("installCompletions", () => {
  ...
  it("installs bash completions", () => {
    ...
    expect(mockFs.mkdirSync).toHaveBeenCalledWith(
      "/home/testuser/.spike/completions",
      { recursive: true },
    );
  ...
  it("does not duplicate source line in rc file", () => {
    ...
    const rcWrites = mockFs.writeFileSync.mock.calls.filter(
      (call: unknown[]) => (call[0] as string).endsWith(".bashrc"),
    );
...
describe("uninstallCompletions", () => {
  ...
  it("cleans up zshrc fpath line on uninstall", () => {
    ...
    const rcWrites = mockFs.writeFileSync.mock.calls.filter(
      (call: unknown[]) => (call[0] as string).endsWith(".zshrc"),
    );
    expect(rcWrites).toHaveLength(1);
