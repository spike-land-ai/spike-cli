...
vi.mock("../../src/multiplexer/upstream-client.js", () => ({
...
import { ServerManager } from "../../src/multiplexer/server-manager.js";
import type { ResolvedConfig } from "../../src/config/types.js";
import type { ToolsetManager } from "../../src/multiplexer/toolset-manager.js";

function cfg(servers: Record<string, object> = {}): ResolvedConfig {
  return { servers: servers as ResolvedConfig["servers"] };
}

function makeMockTsManager(overrides: Partial<{
  ...
  return {
    ...
    handleMetaTool: vi.fn().mockResolvedValue(
      overrides.metaToolResult ?? { content: [{ type: "text", text: "meta" }] },
    ),
...
describe("ServerManager — toolset visibility in getAllTools", () => {
  beforeEach(() => {
    ...
    mockGetTools.mockReturnValue([
      { name: "run", description: "Run", inputSchema: {} },
    ]);
  });
...
describe("ServerManager — callTool with toolset manager", () => {
  ...
  it("routes meta-tool calls through toolset manager handleMetaTool", async () => {
    ...
    expect((ts.handleMetaTool as ReturnType<typeof vi.fn>)).toHaveBeenCalledWith(
      "load_toolset",
      { name: "chess" },
    );
  ...
  it("throws when namespaced tool's server is hidden by toolset", async () => {
    ...
    await expect(manager.callTool("srv__run", {})).rejects.toThrow(
      /toolset not loaded/,
    );
  });
...
describe("ServerManager — callTool filtered tool check", () => {
  ...
  it("throws when the resolved tool name does not exist on the server", async () => {
    ...
    await expect(manager.callTool("srv__nonexistent", {})).rejects.toThrow(
      /Tool not found/,
    );
  });
