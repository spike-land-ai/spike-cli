...
import {
  ...
  parseNamespacedTool,
  stripNamespace,
} from "../../src/multiplexer/namespace.js";
...
describe("namespaceTool", () => {
  ...
  it("uses custom separator", () => {
    expect(namespaceTool("vitest", "run_tests", "::")).toBe(
      "vitest::run_tests",
    );
  });
...
describe("parseNamespacedTool", () => {
  ...
  it("uses greedy matching (longest server name first)", () => {
    ...
    expect(parseNamespacedTool("test_server__do_thing", ambiguousServers))
      .toEqual({
...
describe("stripNamespace", () => {
  ...
  it("returns unchanged if prefix doesn't match", () => {
    expect(stripNamespace("other__run_tests", "vitest")).toBe(
      "other__run_tests",
    );
  });

  it("works with custom separator", () => {
    expect(stripNamespace("vitest::run_tests", "vitest", "::")).toBe(
      "run_tests",
    );
  });
});
