 ...
 */
export function parseInlineServers(
  items: string[],
 ...
 */
export function parseInlineUrls(
  items: string[],
    ...
    try {
      ...
      if (parsed.port !== "") {
        ...
        if (port < 1 || port > 65535) {
          throw new Error(
            `Invalid port ${port} in --server-url "${item}". Port must be 1–65535`,
          );
        }
      }
    } catch (err) {
      // Re-throw our own port validation errors; ignore URL parse errors so
      // that bare hostnames or non-standard schemes still work.
      if (err instanceof Error && err.message.startsWith("Invalid port")) {
        throw err;
      }
    }
